...
description: "Search local design references for a specific palette, type pairing, visual style, UX or accessibility outcome, chart, motion pattern, or stack implementation question. Use when a design decision needs targeted guidance. Supplies candidates to the design lead; does not independently choose a new product direction or run on every frontend edit."
---
...
Follow the [local skill map](../design-lead/reference/skill-map.md). Design Lead leads broad discovery, reference research, concept selection, and review. Search results are recommendations within the selected brief, not rules that replace the product's identity. Existing project tokens remain authoritative during refinements.

## When to Apply

Use this skill when a concrete design or implementation question benefits from its searchable local data. Resolve the question, check that the result fits, and return the useful guidance to the current task. Do not restart discovery or run the database for an already settled component change.
...
1. **An unresolved set of related style decisions** → use `--design-system` to propose a candidate system after reading the brief and references.
...
### Step 2: Propose a Design System (when needed)

Use `--design-system` when the lead needs a coordinated candidate for unresolved visual decisions. Compare its output with the selected references and product goals; it is not an approved design by itself:
...
### Step 2b: Persist Only in the Project’s Chosen Record

First identify the existing source of truth. If the project already uses DESIGN.md or another design record, integrate approved decisions there rather than creating a competing MASTER.md. Use the generator’s persistence only when this is the chosen record and saving it is within the task. Pass `--output-dir` pointed at the project root; otherwise output is relative to the current directory:
...
Return the verified candidates and applicable implementation guidance to the design lead. Implement only within the authorized task and selected direction.
...
- Use `--design-system` for unresolved coordinated choices and `--domain` for a focused concern
