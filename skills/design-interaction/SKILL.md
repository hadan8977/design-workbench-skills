...
Follow the [local skill map](../design-lead/reference/skill-map.md). This specialist implements the selected direction; open product-wide design questions return to design-lead. A scoped component or material request does not restart the full design process.
...
For glass/shader work, this library handles controls, not rendering. Use the available `design-material` skill if relevant; otherwise keep a static surface. Decorative activity must not masquerade as network progress.
