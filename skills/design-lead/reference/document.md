Capture the established design system when documentation is requested or belongs to the authorized substantial design work. Follow the [skill map](skill-map.md) and reuse the existing design record; do not create a parallel specification. The `DESIGN.md` format below applies when that is the chosen project format.
...
- The task calls for a durable design record and no suitable existing record covers it.
- The first implementation of a new world is complete and its provisional decisions need to be carbonized.
- An existing `DESIGN.md` is stale (the design has drifted).
- The user asks to document the current design before a substantial change.

Read the existing record and preserve unrelated decisions. Apply a requested refresh or merge within its authorized scope. Ask only when a conflict or replacement would materially change unresolved design intent; an existing file alone does not require another approval.
...
- **Seed mode**: the project is pre-implementation. Reuse the product brief and new-work's chosen direction to write a provisional design record. A particular product filename is not a prerequisite.

Choose the mode from the actual project and requested deliverable. If no implemented system exists, use seed mode when it matches the request and state that the record is provisional. `$design-lead document --seed` requests new-work's world workshop, but it does not authorize replacing coherent code: when an incumbent system exists, offer scan mode or route an explicit identity-replacement request through new-work.
...
3. Note material assumptions or unresolved facts. No additional confirmation is needed after an authorized update.

Your own write is the freshest source; subsequent commands in this session don't need a reload.

## Seed mode

For projects with no visual system to extract yet. Produces a selected visual-direction scaffold, not a fabricated token spec.

### Step 1: Route through new-work's workshop

Use the product context already established in the conversation or project. Ask only for a missing goal or constraint that changes the direction; do not require a new interview or PRODUCT.md when that context is available.

Use [new-work.md](new-work.md) to frame the problem, research references, and choose a concept if the direction is unresolved. Use the named target or an evident representative surface. If the user delegated the choice, recommend and select it. Stop after the provisional record and surface brief; a documentation request does not authorize implementation.
...
```markdown
<!-- SEED: established with the user before implementation; re-run $design-lead document once there's code to capture the actual tokens and components. -->
```
...
2. Tell the user: "Re-run `$design-lead document` once you have some code. That pass will extract real tokens and generate the sidecar."
...
- **Use concrete principles:** explain reusable decisions plainly. Named rules are optional when they aid recognition; do not invent labels or a quota per section.
...
- Preserve existing decisions outside the authorized update; resolve material conflicts before replacing them.
